# relaxed = run the SOS optimization using the relaxed simpler condition

# sos_zonotope_relaxed - searches for barrier certificate for data driven set of models for discrete time systems
# x(k+1) = Ax(k) + Bu(k) + w(k) using relaxed condition
#
# Other jl-files required: linearDT_relaxed
# Subfunctions: none
...
function bc_relaxed(deg, synthesis = false)
    ...
    # 3rd and last condition for each model
    model,_ = add_psatz!(model, BC-Bf, vars, [g;urange], [], div(maxdegree(BC-Bf)+sos_tol,2), QUIET=true, CS=false, TS=false, Groebnerbasis=true)
...
# Simulation
names = ["linearDT_relaxed"]
system_data = ["vars: ","f: ","go: ","gu: ","g: ","urange: ","drange: "]
max_deg = 6
for name in names
    include("./systems/"*name*".jl"); # load system dynamics info
    f = dyn(x,u,d,Acenter,Bcenter)[1:dim];
    file = open("./systems/"*name*"_system"*string(dim)*".txt", "w"); # open file to write/save system dynamics info
    for (j,k) in zip(system_data, [vars,f,go,gu,g,urange,drange]) # save these data in readable form
        write(file, j*"{")
        for i = 1:length(k)-1
    ...
    # print sufficient verification condition results for standard barrier certificate
    file = open("./systems/"*name*"_bc_dim"*string(dim)*".txt", "w");
    for deg = 1:max_deg
        stats = @timed data = bc_relaxed(deg,false) # time execution
